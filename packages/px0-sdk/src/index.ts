// Vendored from https://github.com/piyushpradhan/px0 @ sdk/index.ts (MIT).
// Kept verbatim except for minimal adaptations for the repo's stricter TS
// settings: `exactOptionalPropertyTypes` (conditional spreads instead of
// `{ signal: undefined }`) and Node's `Response.json(): Promise<unknown>`
// (explicit `<T>` + cast in `get`/`post`). Re-sync by re-copying upstream and
// re-applying those edits.
// This package intentionally opts out of the repo's Effect lint/tsgo rules
// (its own tsconfig), since the SDK is a plain fetch-based client.
//
...
export type Px0Event = { type: "metrics"; data: Metrics } | { type: "git-status"; data: GitStatus };
...
export class Px0 {
  ...
  constructor(opts: Px0Options = {}) {
    ...
    if (base.protocol !== "http:" && base.protocol !== "https:")
      throw new TypeError(`px0 url must be http(s): ${base.href}`);
    ...
    this.getHeaders =
      (opts.compress ?? !isLoopback(base.hostname)) ? {} : { "Accept-Encoding": "identity" };
    this.postHeaders = {
      ...this.getHeaders,
  ...
  private async call<T>(
    ...
    const timer =
      ms > 0
        ? setTimeout(() => ac.abort(timeoutError(`px0 ${path}: no answer after ${ms}ms`)), ms)
        : undefined;
    try {
  ...
  private get<T>(path: string, query?: Query, o?: CallOptions): Promise<T> {
    return this.call<T>("GET", path, query, undefined, o, (r) => r.json() as Promise<T>);
  }

  private post<T>(path: string, query?: Query, body?: unknown, o?: CallOptions): Promise<T> {
    return this.call<T>("POST", path, query, body, o, (r) => r.json() as Promise<T>);
  }
  ...
  private lspGet<T>(path: string, query: Query, o: LspOptions = {}): Promise<T> {
    ...
    const timeout =
      o.timeout ?? (this.timeout && o.wait ? Math.max(this.timeout, o.wait + 5_000) : this.timeout);
    return this.get(
      path,
      { ...query, wait: o.wait },
      {
  ...
  request(
    method: Method,
    path: string,
    o: CallOptions & { query?: Query; body?: unknown } = {},
  ): Promise<Response> {
    ...
    for (let delay = 25; ; delay = Math.min(delay * 2, 500)) {
      try {
        const m = await this.meta({
          ...(o.signal !== undefined ? { signal: o.signal } : {}),
          timeout: deadline === Infinity ? 0 : Math.max(1, deadline - Date.now()),
        });
        if (m.ready) return m;
      } catch (e) {
      ...
      if (Date.now() + delay > deadline)
        throw timeoutError(`px0 at ${this.url} not ready after ${ms}ms`);
  ...
  close = (path: string, o?: CallOptions) =>
    this.get<{ ok: true; path: string }>("close", { path }, o);
  ...
  text = (path: string, o?: CallOptions) =>
    this.call("GET", "raw", { path }, undefined, o, (r) => r.text());
  ...
  search = (q: string, o: SearchOptions & CallOptions = {}) =>
    this.get<SearchResult>(
      "search",
      { q, re: o.regex, case: o.caseSensitive, word: o.word, glob: o.glob },
      o,
    );
  ...
  def = (sym: string, o: CallOptions & { path?: string } = {}) =>
    this.get<DefResult>("def", { sym, path: o.path }, o);

  reindex = (o?: CallOptions) =>
    this.get<{ files: number; indexMs: number; gitChanges: number; gitFiles: string[] }>(
      "reindex",
      undefined,
      o,
    );
  ...
  updateSession = (
    patch: Partial<Pick<Session, "tabs" | "active" | "openDirs">>,
    o?: CallOptions,
  ...
  lsp = {
    ...
    symbols: (path: string, o?: LspOptions) =>
      this.lspGet<LspResult & { symbols: OutlineSymbol[] }>("lsp/symbols", { path }, o),
    ...
    start: (path: string, o?: CallOptions) =>
      this.post<LspBrief>("lsp/start", { path }, undefined, o),
  };
  ...
  git = {
    ...
    stage: (path: string, o?: CallOptions) =>
      this.post<{ ok: true }>("git/stage", undefined, { path }, o),
    unstage: (path: string, o?: CallOptions) =>
      this.post<{ ok: true }>("git/unstage", undefined, { path }, o),
    commit: (message: string, o?: CallOptions) =>
      this.post<{ ok: true }>("git/commit", undefined, { message }, o),
    /** Fast-forward only. Throws Px0Error 409 if diverged or dirty. Talks to the remote: default timeout 5 min. */
    pull: (o?: CallOptions) =>
      this.post<{ ok: true; message: string }>("git/pull", undefined, undefined, {
        timeout: 300_000,
        ...o,
      }),
    /** Talks to the remote: default timeout 5 min. */
    push: (o?: CallOptions) =>
      this.post<{ ok: true }>("git/push", undefined, undefined, { timeout: 300_000, ...o }),
    /** Ask the selected harness for a commit message; stages everything if nothing is staged. Await with agent.wait(). */
    commitMessage: (o?: CallOptions) =>
      this.post<AgentJob>("git/commit-message", undefined, undefined, o),
  };
  ...
  agent = {
    ...
    edit: (edits: EditRequest | EditRequest[], o: CallOptions & { force?: boolean } = {}) =>
      this.post<AgentJob>(
        "agent/batch",
        undefined,
        { edits: Array.isArray(edits) ? edits : [edits], force: !!o.force },
        o,
      ),
  ...
  pr = {
    ...
    comments: (o?: CallOptions) =>
      this.get<{ issueComments: PRComment[]; reviewComments: PRComment[] }>(
        "pr/existing-comments",
        undefined,
        o,
      ),
    drafts: async (o?: CallOptions) =>
      (await this.get<{ comments: DraftComment[] }>("pr/comments", undefined, o)).comments,
    addDraft: (
      c: { path: string; line: number; body: string; side?: "LEFT" | "RIGHT" },
      o?: CallOptions,
    ) => this.post<DraftComment>("pr/comments", undefined, c, o),
    deleteDraft: (id: number, o?: CallOptions) =>
      this.post<{ ok: true }>("pr/comments/delete", { id }, undefined, o),
    /** Submit all drafts as one review. */
    submit: (
      event: "APPROVE" | "REQUEST_CHANGES" | "COMMENT",
      o: CallOptions & { body?: string } = {},
    ) => this.post<{ ok: true }>("pr/submit", undefined, { event, body: o.body ?? "" }, o),
    /** Post a top-level comment immediately. */
    comment: (body: string, o?: CallOptions) =>
      this.post<PRComment>("pr/comments/issue", undefined, { body }, o),
    ...
    launch: (target: string, o?: CallOptions) =>
      this.post<{ ok: true }>("pr/launch", undefined, { target }, o),
  };
}
